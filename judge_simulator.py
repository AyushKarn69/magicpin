...
# Groq settings
LLM_API_KEY = os.getenv("magicpin", "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")
...
# Make the Groq key available to the rest of the script via environment variables
os.environ.setdefault("magicpin", LLM_API_KEY)
...
def load_groq_config() -> Dict[str, str]:
    """Load Groq settings from environment variables and the shared app config."""
    settings = get_settings()
    api_key = os.getenv("magicpin") or settings.openai_api_key or ""

    if not api_key:
        print_fail("Missing Groq API key. Set the magicpin environment variable and retry.")
        sys.exit(1)

    print_success("Groq configuration loaded")
    print_info("Endpoint: https://api.groq.com")
    print_info("Deployment: llama-3.3-70b-versatile")
    print_info("API Version: n/a")
    print_info("Model: llama-3.3-70b-versatile")
    print_info("Run: python judge_simulator.py")
...
class OpenAIProvider(LLMProvider):
    def __init__(self, api_key: str, model: str = "", groq_config: Optional[Dict[str, str]] = None):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        try:
            ...
            if "401" in str(exc) or "unauthorized" in str(exc).lower():
                raise RuntimeError("Authentication failed. Check the Groq API key.") from exc
            if "429" in str(exc) or "rate limit" in str(exc).lower():
                raise RuntimeError("Groq rate limit exceeded. Please retry shortly.") from exc
            if "timeout" in str(exc).lower() or "timed out" in str(exc).lower():
                raise RuntimeError("Groq request timed out. Please retry.") from exc
...
def create_provider(groq_config: Optional[Dict[str, str]] = None) -> LLMProvider:
    """Create LLM provider from configuration."""
    api_key = LLM_API_KEY
    if groq_config:
        api_key = groq_config.get("api_key", api_key)

    providers = {
        "openai": lambda: OpenAIProvider(api_key, LLM_MODEL, groq_config=groq_config),
        ...
        "groq": lambda: OpenAIProvider(api_key, LLM_MODEL, groq_config=groq_config),
...
def main():
    ...
    elif LLM_PROVIDER != "ollama" and not LLM_API_KEY:
    ...
    # Create LLM provider
    try:
        llm = create_provider(groq_config=groq_config)
    ...
    try:
        ...
        print_info("Check your Groq configuration and internet connection")
        sys.exit(1)
